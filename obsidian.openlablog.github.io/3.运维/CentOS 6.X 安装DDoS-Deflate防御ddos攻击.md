DDoS-Deflate是一款非常小巧的防御和减轻DDoS攻击的工具，它可以通过监测netstat来跟踪来创建大量互联网连接的IP地址信息，通过IPTABLES禁止或阻档这些非常IP地址。
...
```
# vim /etc/ddos/ddos.conf　　//脚本和其他文件的路径
```
...
查看定时crond执行任务：/usr/local/ddos/ddos.sh
